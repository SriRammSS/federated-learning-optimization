...
\textbf{Q3. Code location.} \texttt{flopt/fedavg.py}; aggregation is in
...
function \texttt{aggregation\_weights}. The implementation keeps the
baseline sample-size weights and increases mass on clients whose local
loss lies above the $\alpha$-quantile. It is a soft tail reweighting,
not a hard tail-only selector.
