...
An extended remediation pass adds 10 follow-up experiments: SOFA baseline comparison (73{,}141 stays), 30 fresh-seed replication, per-ICU threshold optimization, personalisation baselines, a dense $K\!=\!9$ Dirichlet beta sweep (16 values), CVaR at $K\!=\!30$, architecture/feature/class-weight ablations, and an LP communication sweep (120 FedAvg configs). \textbf{All results are from a single hospital (BIDMC); cross-hospital deployment requires external validation.}

We make four main observations. \textbf{(i)} \fedprox{} with $\mu \!=\! 0.1$ outperforms SOFA-LR on both \auroc{} (+0.161) and \auprc{} (+0.359), providing the clearest case for clinical usefulness in this cohort. On 30 fresh seeds, FedProx-0.1 attains global $\auprc \!=\! 0.665 \pm 0.005$ and worst-client recall $0.513 \pm 0.049$ (95\% CI: $[0.495, 0.531]$). The fairness ranking depends on the chosen metric: on \emph{worst-client} \auprc{}, FedProx-0.01 leads (0.345 vs.\ 0.331 for $\mu\!=\!0.1$); on worst-client recall, FedProx-0.1 leads. We therefore report both metrics throughout. \textbf{(ii)}
...
because updates are only $95.8$--$97.7\%$ nonzero -- the compression mechanism is
\emph{truncation}, not natural sparsity.

We connect each empirical result to a concrete mechanism: \fedprox{} improves
stability by damping per-round drift on the smallest low-mortality unit
(Neuro Intermediate, $1.99\%$ mortality), \cvar{}-$\alpha\!\geq\!0.75$
plateaus because $C\!=\!5$ sampled clients per round leave almost no tail to
reweight ($\lfloor 0.05\!\times\!5 \rfloor = 0$), GA beats grid through an off-grid
learning rate ($\eta \!=\! 0.0029$), and the LP shadow price collapses when the
simplex constraint binds before the budget does. The report also includes a
self-critique log with diffs (Section~\ref{sec:critique}) and a set of
counterfactual rankings (Section~\ref{sec:tables-turned}). All numerical claims
cite a CSV/JSON file under \texttt{outputs/}; all 21 figures are rendered from those CSVs by
