...
\textbf{What to look for:} the floor on the tail size is exactly
$\lfloor (1-\alpha)\,|S_t|\rfloor$, with a fallback to ``keep the
single worst'' (which is the discreteness collapse at $C\!=\!5$).

\begin{lstlisting}
def cvar_aggregator(local_states, local_losses, alpha):
    n = len(local_states)
    if alpha <= 0.0:
        return np.full(n, 1.0 / n)
    k = max(1, int(np.floor((1.0 - alpha) * n)))  # tail size
    order = np.argsort(local_losses)[::-1]        # high-loss first
    tail  = order[:k]
    weights = np.zeros(n)
    weights[tail] = 1.0 / k
    return weights
...
output units (so the same cross-entropy loss applies as for the
MLP); the parameter count is $30 \cdot 2 + 2 = 62$, exactly the
size implied by the $\sim$$248$ B model footprint.
...
\textbf{What to look for:} two hidden layers ($30 \!\to\! 64 \!\to\!
64 \!\to\! 2$) with ReLU and dropout. Parameter count
$30 \cdot 64 + 64 + 64 \cdot 64 + 64 + 64 \cdot 2 + 2 = 6{,}210$.

\begin{lstlisting}
class TabularMLP(nn.Module):
    def __init__(self, input_dim=30, hidden=64, num_classes=2,
                  dropout=0.2):
        super().__init__()
