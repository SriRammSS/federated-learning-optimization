...
def fedprox_train(model:nn.Module,clients:list[ClientData],cfg:FLConfig,mu:float=0.01,
                  track_drift:bool=False,track_sparsity:bool=False)->tuple[nn.Module,list[dict],list[dict]]:
    _set_seed(cfg.seed)
    device=_device()
    ...
    for round_id in range(1,max_rounds + 1):
        ...
        global_reference={key:value.detach().clone() for key,value in global_model.state_dict().items()}

        for client_id in selected:
            local_model=deepcopy(global_model)
            loss=train_one_client_fedprox(local_model,clients[client_id],cfg,device,global_reference,mu)
            state_cpu={key:value.detach().cpu() for key,value in local_model.state_dict().items()}
            if track_sparsity:
                sparsity_rows.extend(update_sparsity_rows(base_state,state_cpu,round_id,client_id,"fedprox",cfg.seed,mu))
...
def train_one_client_fedprox(model:nn.Module,client:ClientData,cfg:FLConfig,device:torch.device,
                             global_ref:dict[str,torch.Tensor],mu:float)->float:
    model.train()
    ...
    last_loss=0.0
    reference={key:value.to(device) for key,value in global_ref.items()}
    ...
    for _ in range(cfg.local_epochs):
        for xb,yb in loader:
            ...
            if mu > 0:
                ...
                # Compare only named params here, otherwise buffers also enter the scene.
                for name,parameter in model.named_parameters():
                    proximal_penalty+=torch.sum((parameter - reference[name]) ** 2)
                loss=loss + 0.5 * mu * proximal_penalty
            loss.backward()
            optimizer.step()
