...
def failure_modes(rows:list[dict])->list[dict]:
    ...
    for row in rows:
        ...
        worst=float(row.get("final_worst_client_accuracy",row.get("worst_client_accuracy",0)) or 0)
        if worst<0.70:
            out.append({**row,"failure_type":"low_worst_client_accuracy","description":"Worst-client accuracy remains below 0.70."})
        if acc<0.90:
            out.append({**row,"failure_type":"low_average_accuracy","description":"Average accuracy remains below 0.90."})
        if row.get("stopped_early") is False and row.get("max_rounds")==row.get("stopped_round"):
            out.append({**row,"failure_type":"no_early_convergence","description":"Run reached max_rounds without early stopping."})
    return out


def selected_case_clients(per_client:list[dict],n:int=5)->list[int]:
...
def summarize_rows(rows:list[dict],group_key:str,metrics:list[str])->list[dict]:
    groups={}
    for row in rows:
