...
def build_logreg_factory(feature_count:int):
    def factory() -> LogisticModel:
        return LogisticModel(feature_count,2)

    return factory


def build_base_config(bundle,settings:dict) -> FLConfig:
    return FLConfig(
...
def main() -> None:
    ...
    try:
        ...
        base=build_base_config(bundle,settings)
        write_metadata(out,args,bundle,base,feature_count)
        runtime.append(stage_row("load",t0))

        mlp_factory=build_mlp_factory(feature_count)
        logreg_factory=build_logreg_factory(feature_count)
        ...
        run_reports(out,args,manifest,bundle)
...
def run_reports(out,args,manifest,bundle):
    ...
    report=alignment_report_text(out,checklist)
...
def alignment_report_text(out,checklist):
    complete=sum(1 for r in checklist if r["status"] == "complete")
    return f"""# MIMIC-IV Proposal Alignment Report

This note summarizes the additional experiments added on top of the existing
MIMIC-IV federated learning pipeline.

## Non-overwrite guarantee

All outputs are written under `{out}`. The existing
`outputs/full_mimic_iv` and `outputs/full_mimic_iv_training` directories are
...
The original MIMIC MLP experiment remains the main clinical run. This alignment
suite adds FedProx, logistic controls, Dirichlet non-IID controls, sparsity and
communication diagnostics, a stronger search pass, loss-landscape plots, and
resource monitoring.
"""
